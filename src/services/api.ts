...
const getResumeUrl = async (payload: ResumeDownloadPayload) => {
    const { data } = await api.post("/resume/download", payload, { responseType: "blob" })
    return window.URL.createObjectURL(data);
}

type AiGenerationContentPayload = {
    jobTitle: string;
    JobDescription: string;
}

const generateContentForJob = async (payload: AiGenerationContentPayload) => {
...
const getPortalUrl = async (currentPathname: string) => {
    const {data} = await api.post<{url:string}>("/credits/transaction",{currentPathname});
    return data.url;
}
